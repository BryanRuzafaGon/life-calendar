...
// ============================================
// ☁️ GUARDAR EN iCLOUD DRIVE
// ============================================

// Persiste el FileSystemDirectoryHandle en IndexedDB para no volver a preguntar
const ICLOUD_DB_NAME = 'lifeCalendarICloud';
const ICLOUD_STORE = 'dirHandle';

function openICloudDB() {
    return new Promise((resolve, reject) => {
        const req = indexedDB.open(ICLOUD_DB_NAME, 1);
        req.onupgradeneeded = e => e.target.result.createObjectStore(ICLOUD_STORE);
        req.onsuccess = e => resolve(e.target.result);
        req.onerror = () => reject(req.error);
    });
}

async function saveICloudDirHandle(handle) {
    const db = await openICloudDB();
    return new Promise((resolve, reject) => {
        const tx = db.transaction(ICLOUD_STORE, 'readwrite');
        tx.objectStore(ICLOUD_STORE).put(handle, 'dir');
        tx.oncomplete = resolve;
        tx.onerror = () => reject(tx.error);
    });
}

async function getICloudDirHandle() {
    const db = await openICloudDB();
    return new Promise((resolve, reject) => {
        const tx = db.transaction(ICLOUD_STORE, 'readonly');
        const req = tx.objectStore(ICLOUD_STORE).get('dir');
        req.onsuccess = () => resolve(req.result || null);
        req.onerror = () => reject(req.error);
    });
}

function setICloudStatus(msg, color = '#555') {
...
async function generateWallpaperCanvas() {
...
document.getElementById('btn-save-icloud').addEventListener('click', async () => {
    // Verificar soporte de la API
    if (!('showDirectoryPicker' in window)) {
        setICloudStatus('⚠️ Tu navegador no soporta esta función. Usa Chrome o Edge.', '#ff9500');
        return;
    }

    const btn = document.getElementById('btn-save-icloud');
    btn.disabled = true;
    btn.textContent = '⏳ Generando wallpaper...';
    setICloudStatus('Generando imagen...', '#888');

    try {
        // Intentar recuperar el directorio guardado
        let dirHandle = await getICloudDirHandle();

        // Verificar permisos si ya teníamos el handle
        if (dirHandle) {
        ...
        // Si no hay handle o perdimos permiso, pedir al usuario que seleccione la carpeta
        if (!dirHandle) {
            setICloudStatus('📂 Selecciona tu carpeta de iCloud Drive...', '#007AFF');
            btn.textContent = '📂 Selecciona la carpeta...';
            dirHandle = await window.showDirectoryPicker({
                id: 'icloud-drive',
                mode: 'readwrite',
                startIn: 'documents'
            });
            await saveICloudDirHandle(dirHandle);
        }

        // Generar la imagen
        btn.textContent = '⏳ Generando wallpaper...';
        setICloudStatus('Renderizando imagen de alta resolución...', '#888');
        ...
        const fileName = `life-calendar-${today}.png`;

        // Escribir en iCloud Drive
        setICloudStatus('☁️ Subiendo a iCloud Drive...', '#007AFF');
        ...
        // ¡Éxito!
        btn.textContent = '✅ Guardado en iCloud';
        setICloudStatus(`✅ "${fileName}" guardado correctamente. Ábrelo desde tu iPhone en Archivos → iCloud Drive.`, '#34c759');
        setTimeout(() => {
            btn.textContent = '☁️ Guardar en iCloud Drive';
            btn.disabled = false;
        }, 3000);
        ...
        if (err.name === 'AbortError') {
            setICloudStatus('Operación cancelada.', '#555');
        } else {
            console.error(err);
            setICloudStatus(`❌ Error: ${err.message}`, '#ff3366');
        }
    }
});
...
function renderHeatmap() {
    ...
    const historyData = JSON.parse(localStorage.getItem('life_journal') || '[]');
    const journalMap = new Map();
    historyData.forEach(entry => {
        // Count non-empty inputs
        const score = (entry.inputs || []).filter(text => text && text.trim() !== '').length;
        if (score > 0) {
            journalMap.set(entry.date, score);
        }
    });
    
    const today = new Date();
    const startDate = new Date(today);
    startDate.setDate(today.getDate() - 364 + 1); // Start 364 days ago
    
    // We want the grid filled column by column (grid-auto-flow: column).
    // The loop inherently adds column by column if we just generate all 364 days in chronological order.
    for (let i = 0; i < 364; i++) {
        ...
        const score = journalMap.get(dateStr) || 0;
        
        if (score > 15) {
            square.classList.add('l-4');
        } else if (score > 10) {
            square.classList.add('l-3');
        } else if (score > 5) {
            square.classList.add('l-2');
        } else if (score > 0) {
            square.classList.add('l-1');
        }
        
        // Tooltip simple: The date
        square.title = `${dateStr}: ${score} manif.`;
        
        heatmap.appendChild(square);
    }
    
    // Scrollear al final para ver hoy
    const wrap = document.querySelector('.heatmap-wrapper');
